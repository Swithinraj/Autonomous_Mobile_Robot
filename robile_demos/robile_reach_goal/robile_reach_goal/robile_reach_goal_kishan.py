    ...
    def timer_callback(self):
        ...
        if not self.direction_found_bool:
            ...
            if self.transform_available:
                ...
                # msg.angular.z = self.get_angular_velocity()
                self.angular_vel = self.get_angular_velocity() 
        ...
        # Set angular velocity in the message
        msg.angular.z = self.angular_vel
        # self.publisher.publish(msg)

    def get_angular_velocity(self):
        """
        Calculate the angular velocity based on the relative angle between the robot's current direction
        and the goal direction.
        """
        goal_angle = math.atan2(self.goal_wrt_base_link.pose.position.y, self.goal_wrt_base_link.pose.position.x)
        print("here is goal angle...",goal_angle)
        current_angle = math.atan2(self.y_vel, self.x_vel)
        print("here is current angle...",current_angle)

        angular_diff = goal_angle - current_angle
        print("here is angular diff...",angular_diff)

        if abs(angular_diff) < math.pi:
            angular_vel = 0.5 * angular_diff
        else:
            angular_vel = 0.5 * (angular_diff - math.copysign(2 * math.pi, angular_diff))
